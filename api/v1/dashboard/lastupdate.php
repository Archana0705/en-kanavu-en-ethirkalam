...
try {
    ...
    if ($getTime->execute()) {
        ...
        http_response_code(400); // Changed from w00 to 400
        echo $response;
        
        // Log query execution failure
        ApiLogger::writeLog(
            'error',
            'Failed to execute get_cronjob_endtime query',
            400,
            $startTime,
            microtime(true),
            $_SERVER['REQUEST_METHOD'],
...
$response = json_encode([
    ...
    http_response_code(400); // Fixed: was w00 (typo)
    echo $response;
    
    // Log database exception
    ApiLogger::writeLog(
        'error',
        'Database exception in get_cronjob_endtime',
        400,
        $startTime,
        microtime(true),
        $_SERVER['REQUEST_METHOD'],
