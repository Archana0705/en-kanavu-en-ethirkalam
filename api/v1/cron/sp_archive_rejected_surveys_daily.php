...
require_once('../../../config/cron_db.php');
...
$logFile = $logBase . 'sp_dashboard_pdssummary.log';
$lockFile = $lockBase . 'sp_dashboard_pdssummary.lock';
...
if (!$fp) {
    ...
    $lockFile = '/tmp/sp_dashboard_pdssummary_' . date('Y-m-d') . '.lock';
    $fp = @fopen($lockFile, 'c');
}
...
try {
    ...
    if (!isset($cron_db_execute) || !$cron_db_execute) {
        throw new Exception("Database connection not established");
    }
    
    // Test connection
    $cron_db_execute->query("SELECT 1");
    
    logMessage("Preparing to execute procedure: sp_dashboard_pdssummary", $logFile);
    
    $benfi_count_sql = "CALL public.sp_archive_rejected_surveys_daily();";
    $benefi_count_stmt = $cron_db_execute->prepare($benfi_count_sql);
    
    if (!$benefi_count_stmt) {
        throw new Exception("Failed to prepare SQL statement: " . 
                          json_encode($cron_db_execute->errorInfo()));
    }
...
?><!-- public.sp_archive_rejected_surveys_daily() -->